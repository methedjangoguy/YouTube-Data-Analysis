...
def getVideoDetails(videIDs):
    ...
    for i in range(0, len(videIDs), 50):
        request = youtube.videos().list(
            part="snippet,statistics", id=",".join(videIDs[i : i + 50])
        )
        response = request.execute()
        try:
            ...
            videoinfo = dict(
                Title=video["snippet"]["title"],
                PublishedDate=pd.to_datetime(video["snippet"]["publishedAt"]).strftime(
                    "%d-%m-%Y"
                ),
                ViewCount=video["statistics"]["viewCount"],
...
# funtion to get a specific channel's playlist details based on channel ID.
...
# function to get a playlist video details based on playlist ID.
def getvideonamesfromplaylist(playlistid):
    request = youtube.playlistItems().list(
    ...
    for i in range(len(response["items"])):
        data = dict(
            VideoName=response["items"][i]["snippet"]["title"],
        )
        videonames.append(data)
